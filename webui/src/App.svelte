<script>
  ...
  function startWS() {
    ...
    ws.onmessage = (m) => {
      ...
      if (ev.kind === 'stats') {
        ...
        if (Array.isArray(ev.food_region) && ev.food_region.length >= 3) foodRegion = ev.food_region;
  ...
  function applyCameraZones(id) {
    ...
    actionPolygon = Array.isArray(decision) && decision.length >= 3
      ? decision
      : null;
    foodRegion = Array.isArray(cam?.food_region) && cam.food_region.length >= 3
      ? cam.food_region
      : null;
